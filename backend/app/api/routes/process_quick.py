...
@router.post("/quick", response_model=ProcessMediaResponse)
async def process_media_quick(
    file: UploadFile = File(...),
    target_languages: str = Form("ru"),  
    generate_tts: bool = Form(False),
    replace_text_on_image: bool = Form(False)
) -> ProcessMediaResponse:
    """
    Быстрая обработка медиа-файла с минимальными опциями
    
    Оптимизации:
    - Минимальная обработка
    - Быстрые настройки моделей
    - Автоматическая фильтрация совпадающих языков
    """
    ...
    try:
        ...
        languages_list = [lang.strip() for lang in target_languages.split(",") if lang.strip()]
        seen = set()
        languages_list = [lang for lang in languages_list if not (lang in seen or seen.add(lang))]
